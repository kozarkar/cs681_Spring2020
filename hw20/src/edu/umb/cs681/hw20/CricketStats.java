...
public class CricketStats {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nCricket Stats with Parallel Streams :");
		...
		Integer maxr = cricketers.stream()
				.parallel()
				.map((CricketStats cricketer)-> cricketer.getRuns()).reduce(0, (result, carp)->
		{
			if(result==0) 
				return carp;
			else if(carp > result) 
		...
		Integer minm = cricketers.stream()
				.parallel()
				.map((CricketStats cricketer)-> cricketer.getMatches()).reduce(0, (result, carp)->
		{
			if(result==0) 
				return carp;
			else if(carp < result) 
		...
		Integer maxm = cricketers.stream()
				.parallel()
				.map((CricketStats cricketer)-> cricketer.getMatches()).reduce(0, (result, carp)->
		{
			if(result==0) 
